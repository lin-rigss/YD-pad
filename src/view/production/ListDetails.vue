<template>
  <div>
          ...
          <div class="info">
            <div class="info-item">
             ...
             <div class="info-item">
              ...
              <div class="value sign">{{goodsName}}</div>
            </div>
    ...
    <div class="btn-footer">
      <button class="common-btn info" @click="submit()">返回</button>
    </div>
  </div>
</template>
...
import {shipmentList , taskUpdate   } from "../../api/goods"
...
export default {
  ...
  mounted(){
     this.idCode = this.$route.params.code;  // 获取二维码的ID码
      ...
      shipmentList().then(res=>{
        ...
        if(res.success ){          
          if(res.data !== null){
            ...
            this.detailData = details.filter(item=>item.codeId ==this.idCode)
            this.isLoading = false;
            // console.log(this.detailData)
            this.type = this.detailData[0].type
            this.getPartsList = this.detailData[0].disassemblyIds;  // 获取零件ID [12, 18]
            this.factoryId = this.detailData[0].factoryId;
            this.goodsId = this.detailData[0].goodsId;   // 货品种类ID  1
            this.goodsName = this.detailData[0].goodsName;
            this.id = this.detailData[0].id;  // 零件自身id 
